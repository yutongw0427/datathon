...
retail['Combined Rate'] = pd.to_numeric(retail['Combined Rate'].str.replace('%', ''),  errors='coerce')

X = pd.get_dummies(retail['industry'], prefix='indus').astype('int')
X2 = pd.get_dummies(retail['state'], prefix='state').astype('int')
X = pd.concat([retail['zipcode'], X,retail[['pop_density', 'wage',  'median_age', 'area', 'unemp_reate', 'Combined Rate']], X2], axis =1)
...
# Tunning the parameters along with cross validation
eta_list = []
max_dep_list = []
col_bytree_list = []
test_rmse_list = []
lam_list = []
for eta in np.arange(0.2, 0.8, 0.2):
    for max_dep in range(4,10,2):
        for col_bytree in np.arange(0.3, 0.7, 0.2):
            for lam in [5, 10, 15]:
                paras = {"objective":"reg:squarederror",
            ...
            cv_results = xgb.cv(dtrain=data_matrix,
                ...
                eta_list.append(eta)
                max_dep_list.append(max_dep)
                col_bytree_list.append(col_bytree)
                col_bytree_list.append(lam)
                test_rmse_list.append(cv_results.mean(axis = 0)[2])
# print([eta, max_dep, col_bytree, lam])
# print(cv_results)
# print(cv_results.mean(axis = 0))

# Pick the final model
xg_reg = xgb.XGBRegressor(objective ="reg:squarederror", colsample_bytree = 0.5, learning_rate = 0.8,
                          max_depth = 8, lamda = 5)
xg_reg.fit(X_train.drop(['zipcode'], axis=1), y_train)
...
predict = xg_reg.predict(all_X.drop(['zipcode'], axis=1))
predict = pd.DataFrame(predict)
predict.columns = ['predict']
ALL_X = pd.concat([all_X, predict], axis = 1)
ALL_X.nlargest(50,'predict').zipcode
